...
function renderTriangles(theta){
	...
	var offset = 3;
	var count = 3;
	// var x = points[0]*Math.cos(theta) - points[1]*Math.sin(theta);
	// var y = points[0]*Math.sin(theta) + points[1]*Math.cos(theta);
	// points[0] = x;
	// points[1] = y;

	/*for(var i=0; i<6; i++) {
		console.log("start"+i+":  "+points[i]+"   ");
	}*/
...
function Affine(theta) {
	...
	for(var i=0; i<points.length; i+=offset) {
		//if( i%offset == 1) {
			x = points[i]*Math.cos(theta) - points[i+1]*Math.sin(theta);
			y = points[i]*Math.sin(theta) + points[i+1]*Math.cos(theta);
